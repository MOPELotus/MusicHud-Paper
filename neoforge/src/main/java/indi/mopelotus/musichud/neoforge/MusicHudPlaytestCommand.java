...
final class MusicHudPlaytestCommand {
    ...
    static void registerClient(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("musichud-tuneweave").then(Commands.literal("playtest")
                .then(Commands.literal("stop").executes(context -> { return DebugPlaytestMessage.stopLocal(); }))
                .then(Commands.argument("identifier", StringArgumentType.greedyString()).executes(context -> {
                    return DebugPlaytestMessage.playLocal(StringArgumentType.getString(context, "identifier"), FormatType.AUTO);
                }))));
    }
