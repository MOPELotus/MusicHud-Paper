...
- **Config**: Built-in TOML config for Fabric/NeoForge (`music_hud-client.toml`, `music_hud-server.toml`) / Paper `config.yml`.
...
- **LavaPlayer** — audio decoder backend, shaded into output jars.
- **Mojang mappings** — `loom.officialMojangMappings()`.

## External Dependencies

- **NCM API Enhanced** — external process (`api`/`api.exe`) managed by `ApiServerManager`. Lifecycle: auto-start on launch, manages process I/O via virtual threads. Deploy to `{corepath}/MusicHud/` for mod builds or `{corepath}/plugins/MusicHud/` for Paper.
