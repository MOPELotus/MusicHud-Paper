...
final class TuneWeaveCloudService {
    ...
    TuneWeaveCloudLibrary loadLibrary() {
        TuneWeavePlatform platform = cloudPlatform();
        List<TuneWeaveCloudTrack> result = new ArrayList<>();
        int offset = 0;
        long total = -1;
        long storageSize = -1;
        ...
        while (true) {
            TuneWeaveApiClient.TuneWeaveResponse response = gateway.requestForPlatform(
                    ...
                    Map.of("platform", platform.apiName(), "limit", "100",
                            "offset", Integer.toString(offset)), null);
            List<JsonElement> page = elements(response.data());
            for (JsonElement value : page) {
                result.add(entities.toCloudTrack(platform, unwrap(value)));
            }
            JsonObject pagination = object(response.meta().get("pagination"));
            JsonObject extensions = object(pagination.get("extensions"));
            if (total < 0) {
                total = longValue(pagination, "total", -1);
            }
            if (storageSize < 0) {
                storageSize = longValue(extensions, "storage_size", -1);
            }
            if (storageMaxSize < 0) {
                storageMaxSize = longValue(extensions, "storage_max_size", -1);
            }
        ...
        return new TuneWeaveCloudLibrary(result, total < 0 ? result.size() : total,
