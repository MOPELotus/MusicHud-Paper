...
public class AccountBaseView extends LinearLayout {
    ...
    public void refresh() {
        ...
        boolean unavailable = (MusicHud.getConnectStatus() != MusicHud.ConnectStatus.CONNECTED
                && !ClientConfig.getInstance().getEnableIsolatedMode()) || !enabled;
        status = unavailable ? Status.UNAVAILABLE : Status.LOGGED;
        removeAllViews();
        ...
        if (status == Status.UNAVAILABLE) {
            ...
            view.addView(new TuneWeaveAccountView(context), new LayoutParams(MATCH_PARENT, MATCH_PARENT));
        }
    }
