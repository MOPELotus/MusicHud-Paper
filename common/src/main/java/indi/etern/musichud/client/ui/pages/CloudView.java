...
public final class CloudView extends LinearLayout {
    ...
    private void selectUpload() {
        ...
        new Modal(getContext(), title(".text.cloud.upload"), form,
                new Modal.ActionButton(I18n.get(MusicHud.MOD_ID + ".button.upload"), (button, modal) -> {
                    modal.dismiss();
                    mutate(".text.cloud.uploading", () -> tuneWeave.uploadCloudTrack(
                            Path.of(path), text(song), text(artist), text(album)));
                }), cancel()).show();
    }
    ...
    private void selectDownload(TuneWeaveClientService.CloudTrackInfo cloudTrack) {
        ...
        if (path == null || path.isBlank()) return;
        mutate(".text.cloud.downloading", () -> tuneWeave.downloadCloudTrack(cloudTrack, Path.of(path)));
    }
        ...
        new Modal(getContext(), title(".text.cloud.import"), form,
                new Modal.ActionButton(I18n.get(MusicHud.MOD_ID + ".button.import"), (button, modal) -> {
                    try {
                        ...
                        modal.dismiss();
                        mutate(".text.cloud.saving", () -> tuneWeave.importCloudTrack(text(md5), text(source),
                                bitrateValue, sizeValue, text(type), text(song), text(artist), text(album)));
    ...
    private void showMatch(TuneWeaveClientService.CloudTrackInfo cloudTrack) {
        ...
        new Modal(getContext(), title(".text.cloud.match"), target,
                new Modal.ActionButton(I18n.get(MusicHud.MOD_ID + ".button.confirm"), (button, modal) -> {
                    modal.dismiss();
                    mutate(".text.cloud.saving", () -> tuneWeave.matchCloudTrack(cloudTrack, text(target)));
                }), cancel()).show();
    }
    ...
    private void showLyrics(TuneWeaveClientService.CloudTrackInfo cloudTrack) {
        ...
        MusicHud.EXECUTOR.execute(() -> {
            try {
                LyricInfo lyrics = tuneWeave.loadCloudLyrics(cloudTrack);
                String advanced = lyrics.getWordByWordLyric().getLyric();
                String content = advanced.isBlank() ? lyrics.getLyric().getLyric() : advanced;
                MuiModApi.postToUiThread(() -> showTextModal(cloudTrack.track().getName(), content));
            } catch (RuntimeException error) {
                MuiModApi.postToUiThread(() -> showStatus(message(error)));
            }
        });
    ...
    private void confirmDelete(TuneWeaveClientService.CloudTrackInfo cloudTrack) {
        ...
        new Modal(getContext(), title(cloudTrack.track().getName()), warning,
                new Modal.ActionButton(I18n.get(MusicHud.MOD_ID + ".button.delete"), (button, modal) -> {
                    modal.dismiss();
                    mutate(".text.cloud.saving", () -> tuneWeave.deleteCloudTrack(cloudTrack));
                }), cancel()).show();
    }

    private void mutate(String pendingKey, Runnable operation) {
