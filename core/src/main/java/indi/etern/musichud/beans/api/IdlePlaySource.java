    ...
    public void serverLoadMusicCollection(UUID playerUUID) {
        if (musicCollection == null) {
            if (type.equals(Album.class)) {
                dataLoaded = true;
                musicCollection = IMusicApiService.getInstance(ApiProvider.TUNEWEAVE).getAlbumInfoDetail(id, playerUUID);
            } else if (type.equals(Playlist.class)) {
                dataLoaded = true;
                musicCollection = IMusicApiService.getInstance(ApiProvider.TUNEWEAVE).getPlaylistDetail(id, playerUUID);
            }
        }
    }
